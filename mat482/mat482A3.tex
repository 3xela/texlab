...
We see that 
		$$\left|D^2v_m(x_m) - D^2 v_m(y_m) \right| = M^{-1}r_m^{-\alpha}\left|D^2u_m(p_m) - D^2u_m(y_m) \right| = 1.$$
...
\end{enumerate}
\item By $iib)$ we have a uniform $C^{2,\alpha}$ bound on the sequence $\{u_m\}$. We can therefore pass to a uniformly convergent subsequence, and relable it as $\{u_m\}$. Since $M^{-1} \to 0$ by $iiv)$, we have that $a^{ij}(v_m(x))_{ij} \to 0$. Therefore the limit function $v_\infty$ is harmonic.
	Furthermore as $M^{-1} \to 0$ we have that $v_\infty(x) = 0$  when $x_n =0$ by looking at the boundary. Finally note that the this locally looks like the half plane, since we can zoom in sufficiently close on the boundary of the ball. Therefore we can apply the Schwartz reflection principle and get a harmonic which is $0$ on $x_n =0$. But by part $iic)$ we have that 
...
\end{problem}
\penum
\item We proceed by induction. For $k=0$, since $f\in C^\alpha(\ol{B})$, we have that $u\in C^{2, \alpha}(\ol{B})$. By theorem 0.4 we have that
	$$\norm{u_0}_{C^{2, \alpha}} \leq C \left( \norm{f}_{C^{\alpha}(\ol{B})} + \norm{u_0}_{C^0 (\ol{B}} \right). $$
Since $a^{ij} = \delta_{ij}$, and $\lambda = 1$ we have that $C$ only depends on $n$. We have proven in class that the following bound holds: 
$$\norm{u_0}_{C^0(B)} \leq \norm{u|_{\bd B}}_{\bd B} + C \norm{f}_{C^0(B)}.$$
Since $u_0$ vanishes along the boundary, we have that 
$$\norm{u_0}_{C^0(\ol{B})}\leq C \norm{f}_{C^0(B)} \leq C\norm{f}_{C^\alpha(\ol{B})}.$$
Note that $C$ must only depend on $n$ since once again we are dealing with the laplace equation. 
Therefore we must have that 
$$\norm{u_0}_{C^{2,\alpha} (\ol{B})} \leq C (1+ \Lambda)\norm{f}_{C^\alpha(\ol{B})}$$
where $\Lambda$ is the ellipticity constant satisfying $\lambda |\xi|^2 \leq (\delta_{ij} - a^{ij})\xi_i \xi_j \leq \Lambda |\xi^2|$ for $L$. 
...
$$\norm{u_{k+1}}_{C^{2,\alpha}(\ol{B})}\leq C(1+\Lambda)\norm{u_k}_{C^{2,\alpha}(\ol{B})}\leq C^{k+1}(1+\Lambda)^{k+1}\norm{f}_{C^{\alpha}(B)}.$$
\item We can write: 
$$L_t v_k = $$
\item 
\item 
