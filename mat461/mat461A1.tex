...
Since $G_t,G_t$ preserve norms, both sides must have norms of $\norm{x}$. So 
$$\norm{x} = \norm{G_t x} = \norm{G_0 + yt} = \norm{G_0x}.$$
...
Note that the way $g(x,t)$ is written, it is the composition of 
\newpage
\begin{problem}
\end{problem}
...
\begin{enumerate}
	...
	\item As $y$ varies for fixed $x_i$, so that the $x$ component of our vector field changes. 
\end{enumerate}
