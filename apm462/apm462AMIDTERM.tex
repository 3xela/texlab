...
We relabel as $\tilde{a}, \tilde{b}$ so that $\tilde{a}^T x_0 <0, \tilde{b}^Tx<0$ by multiplying $a,b$ by $-1$ or $1$. 
...
Since $\bd C$ is not convex, and the set of minimums form a convex set, the minimum must be exactly $x^*$. 
\epenum
\newpage
\begin{problem}
\end{problem}
