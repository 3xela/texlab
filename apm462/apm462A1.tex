...
The necessary condition for local minima is $H(f)\geq 0$. Sylvestors criterion tells us this happens exactly when $4(\alpha+1)\geq 0, 2\alpha^2(\alpha+1) - 25 \geq0$. This is satisfied when $\alpha \geq 1.383$.   
\epenum
\newpage
\begin{problem}
\end{problem}
...
This is satisfies at $x = 3,y=5$. This is not in the domain however. We check the boundary for a minimum. Let $\gamma(t) = (\cos t, \sin t)$ on $[0,2\pi]$. We wish to minimize $f\circ \gamma$. Note that this will attain a minimum, since it is a compact set. 
\end{document}
